...
from openhands_server.runtime_container.runtime_container_service import RuntimeContainerService
from openhands_server.utils.success import Success

router = APIRouter(prefix="/runtime-containers")
runtime_service: RuntimeContainerService = get_impl(RuntimeContainerService)()
router.lifespan(runtime_service)
...
async def search_runtime_containers(page_id: str | None = None, limit: int = 100, user_id: UUID = Depends(get_user_id)) -> RuntimeContainerPage:
    assert limit > 0
    assert limit <= 100
    return await runtime_service.search_runtime_containers(user_id, page_id, limit)


@router.get("/{id}")
async def get_runtime_containers(id: UUID, user_id: UUID = Depends(get_user_id)) -> RuntimeContainerInfo:
    runtime_containers = await runtime_service.get_runtime_containers(id)
...
async def batch_get_runtimes(ids: list[UUID], user_id: UUID = Depends(get_user_id)) -> list[RuntimeContainerInfo | None]:
    assert len(ids) < 100
    runtime_containerss = await runtime_service.batch_get_runtime_containers(user_id, ids)
    runtime_containerss = [
...
async def start_runtime(user_id: UUID = Depends(get_user_id)) -> UUID:
    id = await runtime_service.start_runtime(user_id)
    return id


@router.post("/{id}/pause")
async def pause_runtime(id: UUID, user_id: UUID = Depends(get_user_id)) -> Success:
    exists = await runtime_service.pause_runtime(user_id, id)
    if not exists:
        raise HTTPException(status.HTTP_404_NOT_FOUND) 
    return Success()
...
async def resume_runtime(id: UUID, user_id: UUID = Depends(get_user_id)) -> Success:
    exists = await runtime_service.resume_runtime(user_id, id)
    if not exists:
        raise HTTPException(status.HTTP_404_NOT_FOUND) 
    return Success()
...
async def delete_runtime(id: UUID, user_id: UUID = Depends(get_user_id)) -> Success:
    exists = await runtime_service.delete_runtime(user_id, id)
    if not exists:
        raise HTTPException(status.HTTP_404_NOT_FOUND) 
    return Success()